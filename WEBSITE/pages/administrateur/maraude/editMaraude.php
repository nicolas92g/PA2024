        ...
        <div class="row">
            <div class="col-12 text-center py-4">
                <div class="d-flex justify-content-between p-3">
                    ...
                    <button class="btn btn-secondary" onclick="deleteMaraude()">Supprimer</button>
                </div>
                ...
                <button class="btn btn-primary m-5" data-bs-toggle="modal" data-bs-target="#exampleModal">Ajouter un produit</button>
            ...
            <div class="d-flex">
                ...
                <div class="col-3 d-flex flex-column align-items-center">
                    ...
                    <button class="btn btn-outline-primary my-4" data-bs-toggle="modal" data-bs-target="#newStepModal">Nouvelle Etape</button>

                    <!-- Modal -->
                    <div class="modal fade" id="newStepModal" tabindex="-1" aria-labelledby="newStepModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="newStepModalLabel">Nouvelle Etape</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <label class="form-label">Addresse</label>
                                    <input class="form-control" type="text" id="adresseStep">

                                    <label class="form-label">Code postal</label>
                                    <input class="form-control" type="text" id="codePostalStep">

                                    <label class="form-label">Ville</label>
                                    <input class="form-control" type="text" id="villeStep">
                                </div>
                                <div class="modal-footer">
                                    <button class="btn btn-primary" onclick="newStep()">Valider</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    response.json().then((etapes) => {
                                        ...
                                        for (const etape of etapes) {
                                            ...
                                            table.innerHTML += '<tr>' +
                                                '<td>' + i + '</td>' +
                                                '<td>' + address + '</td>' +
                                                '<td><button class="btn btn-sm btn-outline-primary" onclick="deleteStep(' + etape.maraude + ', ' + etape.addresse + ')">Supprimer</button></td>' +
                                                '</tr>';
                                            i++;
                                        }
                                        updateMap();
            ...
            response.json().then((productList) => {
                ...
                for (const product of notusedProducts) {
                    table.innerHTML += '<tr><th scope="row">' + product.id + '</th>' +
                        ...
                        '<td><button class="btn btn-outline-primary" onclick="addProduct(' + product.id + ')">ajouter</button></td>'
                    '</tr>';
                }
                ...
                for (const product of products) {
                    table.innerHTML += '<tr><th scope="row">' + product.id + '</th>' +
                        ...
                        '<td><button class="btn btn-outline-primary" onclick="removeProduct(' + product.id + ')" >enlever</button></td>'
                    '</tr>';
                }
            })
        })
